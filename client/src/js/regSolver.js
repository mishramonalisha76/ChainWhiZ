...
export default async function regSolverVerify() {
let output=null
...
const amt = window.web3.utils.toWei('0.05','ether')
 ...
 async function regSolver()
 {
    ...
    else
    {
        ...
        if  (trnOutput)
        {
            await contract.methods.addSolver().send({ from: account[0] ,chainId: 3})
            output = await contract.methods.verifySolver().call({ from: account[0] ,chainId: 3});
            console.log(output)
        }
